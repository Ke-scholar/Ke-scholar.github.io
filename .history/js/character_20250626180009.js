document.addEventListener('DOMContentLoaded', function () {
    ...
    const characterId = urlParams.get('category');

    // 获取分类信息
    const character  = getCategoryById(characterId);
    if (!character ) {
    ...
    // 设置标题和描述
    document.getElementById('category-title').textContent = character.name;
    document.getElementById('category-desc').textContent = character.description;

    // 获取图片数据
    const galleryGrid = document.getElementById('category-gallery');
    const loadMoreBtn = document.getElementById('load-more-btn');

    const allImages = getAllImagesByCategory(characterId);
    let visibleItems = allImages.length;
